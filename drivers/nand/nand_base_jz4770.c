...
static struct nand_oobinfo nand_oob_436 = {
	.useecc = MTD_NANDECC_AUTOPLACE,
	.eccbytes = 104,
	.eccpos = {0},
	.oobfree = {{24,412}}
...
#else // (CFG_NAND_BCH_BIT == 4) /* CFG_NAND_BCH_BIT == 4 */
static struct nand_oobinfo nand_oob_64 = {
	.useecc = MTD_NANDECC_AUTOPLACE,
	.eccbytes = 28,
	.eccpos = {
	...
	u_char *oob_buf,  struct nand_oobinfo *oobsel, int cached)
{
	...
	int	datidx = 0, eccidx, eccsteps = this->eccsteps;
	int	eccbytes = 0;
	int	cnt = 0;

#ifdef CFG_NAND_BCH_WITH_OOB
	eccidx = this->eccbytes;
#else
	eccidx = 0;
#endif
	...
	case NAND_ECC_HW39_512:
		eccbytes = this->eccbytes;
		unsigned char *free_p , free_buf[NAND_MAX_FREESIZE];
		free_p = free_buf;
		memset(free_p, 0x0, NAND_MAX_FREESIZE);
		for (cnt = 0;cnt < eccsteps; cnt++) {
...
#ifdef CFG_NAND_BCH_WITH_OOB
				if ((CFG_NAND_ECC_POS + (cnt + 1) * eccbytes + i) < mtd->oobsize)
					oob_buf[CFG_NAND_ECC_POS + (cnt + 1) * eccbytes + i] = ecc_code[i];
				else
					*free_p++ = ecc_code[i];
#else
				if ((CFG_NAND_ECC_POS + cnt * eccbytes + i) < mtd->oobsize)
					oob_buf[CFG_NAND_ECC_POS + cnt * eccbytes + i] = ecc_code[i];
				else
					*free_p++ = ecc_code[i];
#endif
			}
		...
		this->write_buf(mtd, free_buf, mtd->freesize);
		break;
			     ...
			     struct nand_oobinfo *oobsel)
{
	...
	u_char *free_pos = NULL;
	u_char ecc_calc[NAND_MAX_ECC];
	u_char ecc_code[NAND_MAX_ECC];
	...
	while (read < len) {
		...
		this->read_buf(mtd, data_poi, mtd->oobblock + mtd->oobsize);
		oob_data = &data_poi[mtd->oobblock];
		free_pos = data_poi + mtd->data_per_page;
		...
		if(NAND_ECC_HW9_512 == eccmode){
			for (i = 0, datidx = 0; i < this->eccsteps; i++, datidx += ecc) {
				if(correct_flag)
					this->enable_hwecc(mtd, NAND_ECC_READ);
				this->read_buf(mtd, &data_poi[datidx], ecc);
		...
		} else if ((NAND_ECC_HW13_512 == eccmode) || (NAND_ECC_HW7_512 == eccmode) || (NAND_ECC_HW39_512 == eccmode))  {
			memset(ecc_code,0xff,mtd->oobsize + mtd->freesize);
			memcpy(ecc_code,&data_poi[end + CFG_NAND_ECC_POS],mtd->oobsize - CFG_NAND_ECC_POS);
			if(mtd->freesize != 0){
				memcpy(&ecc_code[mtd->oobsize - CFG_NAND_ECC_POS],free_pos,mtd->freesize);
			}
			for (i = 0, datidx = 0; i < this->eccsteps; i++, datidx += ecc) {
				this->enable_hwecc(mtd,NAND_ECC_READ);
				ecc_status = this->correct_data(mtd, &data_poi[datidx], &ecc_code[j],NULL);
...
static u_int32_t calc_free_size(struct mtd_info *mtd)
{
	struct nand_chip *this = (struct nand_chip *)mtd->priv;
	u_int32_t freesize;
	u_int32_t pagesize = mtd->oobblock;
	u_int32_t oobsize = mtd->oobsize;
	u_int32_t eccsize = this->eccsize;
	u_int32_t eccbytes = this->eccbytes;

#ifdef CFG_NAND_BCH_WITH_OOB
	if ((pagesize / eccsize + 1) * eccbytes + CFG_NAND_ECC_POS > oobsize)
		freesize = NAND_MAX_FREESIZE;
	else
		freesize = 0;
#else
	if ((pagesize / eccsize) * eccbytes + CFG_NAND_ECC_POS > oobsize)
		freesize = NAND_MAX_FREESIZE;
	else
		freesize = 0;
#endif
...
int nand_scan (struct mtd_info *mtd, int maxchips)
{
	...
	*/
...
#if defined(CONFIG_NAND_K9GAG08U0E) || defined(CONFIG_NAND_K9GBG08U0M)
	this->page_shift = 13;
	this->phys_erase_shift = 20;
	this->chipsize = 2048 << 20;
	mtd->erasesize = 1 << 20;
	...
	mtd->data_per_page = mtd->oobblock - mtd->freesize;
	...
	case NAND_ECC_HW13_512:
	case NAND_ECC_HW39_512:
		this->eccsteps = mtd->data_per_page / 512;
		break;
	case NAND_ECC_HW3_256:
	case NAND_ECC_SOFT:
